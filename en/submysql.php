...
// v4:  $out_connection = mysql_connect("localhost","sqluser","sssqql") or die ("Could not connect to the server: " . mysql_error());
   $out_connection = mysqli_connect("localhost","cumc2012","a3[We^1t") or die ("Could not connect to the server: ");
...
// v4: $out_db = mysql_select_db($in_db, $in_connection) or die ("Could not select database: " . mysql_error());	
   $out_db = mysqli_select_db($in_connection, $in_db) or die ("Could not select database: ");	
   return $out_db;
}
...
// default dbname can be provided here as 4th parameter
  $my_connection = mysqli_connect("localhost","cumc2012","a3[We^1t") or die ("Could not connect to the server: ");
  if (mysqli_connect_errno()) {
      printf("Connect failed: %s\n", mysqli_connect_error());
      exit();
   }
// v4: $dummy_db = mysql_select_db($in_db2, $my_connection) or die ("Could not select database: " . mysql_error());
  $dummy_db = mysqli_select_db($my_connection, $in_db2) or die ("Could not select database: ");
